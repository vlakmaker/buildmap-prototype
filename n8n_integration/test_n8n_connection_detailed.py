...
def test_workflow_creation():
    ...
    test_workflow = {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "parameters": {
                    "values": {"string": [{"name": "test", "value": "success"}]}
                },
                "position": [450, 300],
                "id": "set-variable",
            },
    ...
    try:
        ...
        response = requests.post(
            workflow_url, headers=headers, json=test_workflow, timeout=15, verify=True
        )
        ...
        if response.status_code == 200:
            ...
            print_info(
                f"Direct URL: {N8N_BASE_URL.replace('/rest', '')}/workflow/{workflow['id']}"
            )
            ...
            if delete_response.status_code == 200:
                ...
                print_warning(
                    f"Could not delete test workflow: {delete_response.status_code}"
                )
...
def show_configuration():
    ...
    print(
        f"{Colors.BOLD}N8N_API_KEY:{Colors.ENDC} {'Set' if N8N_API_KEY else 'Not set'}"
    )
